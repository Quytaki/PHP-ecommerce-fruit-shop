...
class adminback
{
    ...
    function display_product()
    {
        ...
        $result = mysqli_query($this->connection, $query);
        if ($result === false) {
            echo '<div style="color:red;text-align:center;">Lỗi SQL: ' . mysqli_error($this->connection) . '</div>';
            return false;
        }
        return $result;
    }
    ...
    function view_all_product()
    {
        ...
        // Debug số lượng sản phẩm lấy được
        // echo "<div style='color:blue;text-align:center;'>Số sản phẩm lấy được: " . mysqli_num_rows($result) . "</div>";
        return $result;
    }
